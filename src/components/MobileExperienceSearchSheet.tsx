...
import { Users, Calendar, DollarSign, ChevronDown, ChevronUp, Minus, Plus, Search } from 'lucide-react';
...
interface MobileExperienceSearchSheetProps {
  onSearch: (filters: {
...
export function MobileExperienceSearchSheet({ onSearch, onClose }: MobileExperienceSearchSheetProps) {
  ...
  const hasAnyFilters = selectedFilters.length > 0 || numberOfPeople || tripDays || startDate || endDate || minBudget || maxBudget || priceRating !== null;
  ...
  return (
    <div className="flex flex-col h-full max-h-[85vh]">
            ...
            >
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-slate-900 flex items-center justify-center">
                  <PlaceSettingIcon className="w-7 h-7 text-white" />
                </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 bg-white border-t p-4 shadow-lg">
        ...
        >
          <PlaceSettingIcon className="w-5 h-5 mr-2" />
          Search Destinations
          {selectedFilters.length > 0 && (
